...
/* Define an enum to configure the pins of the LCD */
...
/* Define an enum to configure the ports for each pin */
typedef enum {

	LCD_RS_Port = PORTC,               //configure the keypad's port of RS pin as port x.
	LCD_RW_Port = PORTC,               //configure the keypad's port of RW zero pin as port x.
	LCD_Enb_Port = PORTC,              //configure the keypad's port of ENB zero pin as port x.
	LCD_Data_Zero_Port = PORTC,        //configure the keypad's port of data zero zero pin as port x.
	LCD_Data_One_Port = PORTC,         //configure the keypad's port of data one pin as port x.
	LCD_Data_Two_Port = PORTC,         //configure the keypad's port of data two pin as port x.
	LCD_Data_Three_Port = PORTC,       //configure the keypad's port of data three pin as port x.
	LCD_Data_Four_Port = PORTC,        //configure the keypad's port of data four pin as port x.
	LCD_Data_Five_Port = PORTC,        //configure the keypad's port of data five pin as port x.
	LCD_Data_Six_Port = PORTC,         //configure the keypad's port of data six pin as port x.
	LCD_Data_Seven_Port = PORTC        //configure the keypad's port of data seven pin as port x.

}LCD_Ports;  //the end of the enum

/* Define an enum to configure the commands of the LCD */
typedef enum {
...
/* Define an enum to define the rows of the LCD */
typedef enum
{
...
/* Define an enum to define the LCD's columns */
typedef enum
{
...
/* Define an enum to define CGRAM's locations */
typedef enum
{
...
/* Define a structure for Character generator configuration parameters  */
typedef struct
{
	LCD_Patterns_Address No_Address;   //Define a variable from type LCD_Patterns_Address
	u8 *Pattern;                       //Define a pointer to hold the patterns
	LCD_Rows Row_Position;             //Define a variable from type LCD_Rows
	LCD_Columns Column_Position;       //Define a variable from type LCD_Columns
	u8 Loop;                           //Define a variable to use it in a for loop
...
void __LCD_Send_String_Position__(u8 *Data,u8 Row,u8 Column);       //the prototype of the function that sends a string to the LCD with set position
