...
typedef struct
{
	u8 MUX : 5;     //Declare a variable with 5bit size as a MUX bit
	u8 ADLAR : 1;   //Declare a variable with 1bit size as a ADLAR bit
	u8 REFS :2;     //Declare a variable with 2bit size as a REFS bit

}ADMUX_Bits;   //the end of the structure
...
typedef struct
{
	u8 ADPS : 3;    //Declare a variable with 3bit size as a ADPS bit
	u8 ADIE : 1;    //Declare a variable with 1bit size as a ADIE bit
	u8 ADIF : 1;    //Declare a variable with 1bit size as a ADIF bit
	u8 ADATE : 1;   //Declare a variable with 1bit size as a ADATE bit
	u8 ADSC : 1;    //Declare a variable with 1bit size as a ADSC bit
	u8 ADEN : 1;    //Declare a variable with 1bit size as a ADEN bit

}ADCSRA_Bits;  //the end of the structure
...
typedef struct
{
	u8 PSR10 : 1;     //Declare a variable with 1bit size as a PSR10 bit
	u8 PSR2 : 1;      //Declare a variable with 1bit size as a PSR2 bit
	u8 PUD : 1;       //Declare a variable with 1bit size as a PUD bit
	u8 ACME : 2;      //Declare a variable with 2bit size as a ACME bit
	u8 ADTS : 3;      //Declare a variable with 3bit size as a ADTS bit

}SFIOR_Bits;  //the end of the structure
...
{
	u8 Register_Access;         //Declare a variable with the size of the register
	SFIOR_Bits Bit_Access;       //Declare a variable from type SFIOR_Bits

}SFIOR_Access;  //the end of the union
...
{
	u8 Register_Access;       //Declare a variable with the size of the register
	ADCSRA_Bits Bit_Access;    //Declare a variable from type ADCSRA_Bits

}ADCSRA_Access; //the end of the union
...
{
	u8 Register_Access;      //Declare a variable with the size of the register
	ADMUX_Bits Bit_Access;    //Declare a variable from type ADMUX_Bits

}ADMUX_Access; //the end of the union

/* Declare a structure to access ADCHL, ADCSRA and ADMUX registers */

typedef struct
{
	volatile u16 ADCHL;               /* Declare the ADCHL register with a size if ADCL and ADCH  8bit + 8bit = 16bit  */
	volatile ADCSRA_Access ADCSRA;    /* Declare the ADCSRA register */
	volatile ADMUX_Access ADMUX;      /* Declare the ADMUX register */

}ADC_Registers;   //the end of the structure




/* Declare a structure to access SFIOR registers */

typedef struct
{
	volatile SFIOR_Access SFIOR;     /* Declare the SFIOR register */

}ADC_Trigger_Register;   //the end of the structure



#define ADC  ((ADC_Registers*)0x24)                     //Define the base address of the ADC registers
#define ADC_Trigger  ((ADC_Trigger_Register*)0x50)     //Define the base address of the SFIOR register
